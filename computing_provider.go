...
func (c *ComputingProviderController) Add(ctx *app.AddComputingProviderContext) error {
	...
	transactionHash, err := transaction.OperateTransaction(transaction.ADD, ctx.Hash, ctx.PrivateKey, config)
	if err != nil {
		return ctx.InternalServerError(
...
func (c *ComputingProviderController) Del(ctx *app.DelComputingProviderContext) error {
	...
	transactionHash, err := transaction.OperateTransaction(transaction.DEL, ctx.Hash, ctx.PrivateKey, config)
	if err != nil {
		return ctx.InternalServerError(
			goa.ErrInternal("operateTrasaction failure"))
	}
...
// // generate generate, sign, send transaction, return hash
